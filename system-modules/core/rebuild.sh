...
git reset > /dev/null 
if [ "$TYPE" = "system" ]; then
  git add ./hosts/* ./system-modules/* ./assets/* ./flake.nix ./flake.lock ./.gitignore ./.sops.yaml ./secrets/* ./readme.md
  ...
  git add ./home/* ./assets/* ./flake.nix ./flake.lock ./.gitignore ./.sops.yaml ./secrets/* ./readme.md
  git --no-pager diff -U0 --staged .
