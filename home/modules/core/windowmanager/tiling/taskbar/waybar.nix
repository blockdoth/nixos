...
let
  module = config.modules.core.windowmanager.tiling.taskbar.waybar;
    ...
    programs.waybar = {
      ...
      settings = {
        mainBar = {
          ...
          "custom/piss" = {
            format = "| <span font='${toStr (icon-size * 0.7)}' rise='${toStr (v-offset * 0.0)}pt'>󰦫</span>  {}% ";
          ...
          pulseaudio = {
            ...
            format-source = "| <span font='${toStr icon-size}' rise='${toStr (v-offset * 1)}pt'></span> ";
            format-source-muted = "|  ";
            format-icons = {
              headphone = "";
              default = [
